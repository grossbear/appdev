...
{
public:
    bool active = false; // Window active flag
    bool fullscreen = false; // Window fullscreen flag
    bool blockScreensaver = false;
    bool blockMonitorPowerSave = false;

    bool closeOnEsc = true; // Close window on escape key
};

WinParams gWinParams;

///////////////////////////////////////////////////////////////////////////////
AppWindow::AppWindow(int x, int y, int width, int height, int flags, const char * wname):
hWnd(NULL) 
{
    ...
    strutils::cat(winClassName, WIN_CLASS_SUFFIX, sizeof(WIN_CLASS_SUFFIX));

    Create(x, y, width, height, flags);
}
...
LRESULT CALLBACK AppWindow::WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch(message)
    {
        ...
        case WM_ACTIVATE:
        {
            ...
            app->OnWindowActive(gWinParams.active);
			return 0;		
        }
        break;
        ...
        case WM_LBUTTONDBLCLK:
        {
            ...
            if(message == WM_LBUTTONDOWN)
                app->OnMouseButtonDown(btype, x, y);
            else if(message == WM_LBUTTONUP)
                app->OnMouseButtonUp(btype, x, y);
            else
                app->OnMouseButtonDblClk(btype, x, y);
            return 0;
        }
        break;
        ...
        case WM_MOUSEMOVE:
        {
            ...
            {
            case MK_LBUTTON:
                app->OnMouseMove(BTYPE_LBUTTON, x, y);
            break;

            case MK_RBUTTON:
                app->OnMouseMove(BTYPE_RBUTTON, x, y);
                break;

            case MK_MBUTTON:
                app->OnMouseMove(BTYPE_MBUTTON, x, y);
                break;
            }
            return 0;
        }
